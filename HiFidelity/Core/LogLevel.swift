...
final class Logger {
    ...
    private init() {
        // Initialize the dispatch queue for file logging
        self.logQueue = DispatchQueue(label: "com.hifidelity.logger.queue", qos: .utility)
        // Initialize unified logging with subsystem and category for better filtering
        let subsystem = Bundle.main.bundleIdentifier ?? "com.hifidelity"
        self.osLog = OSLog(subsystem: subsystem, category: "Logger")
        // Ensure logs directory exists
        fileManager.createLogDirectoryIfNeeded()
        // Optionally perform rotation on startup to trim old entries
...
private final class LogFileManager {
    ...
    private let logFileName: String = {
        // Use a simple, stable log file name. If desired, include bundle identifier for clarity.
        return "app.log"
    }()
